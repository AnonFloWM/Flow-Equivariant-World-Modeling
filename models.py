...
class FERNN_Cell(nn.Module):
    ...
    def forward(self, u_t, h_prev, action):
        ...
        ## Optional vectorized alternative to roll_batch (currently not used)
        # shift hidden via torch.roll based on action
        # h_shift = torch.roll(h_prev, shifts=(action[1], action[0]), dims=(2, 3))
        # dx = action[:, 0].view(B, 1, 1, 1)        # [B,1,1,1]
        # dy = action[:, 1].view(B, 1, 1, 1)

        # # Create base indices once
        # grid_y = torch.arange(H_hidden, device=h_prev.device).view(1, 1, H_hidden, 1)
        # grid_x = torch.arange(W_hidden, device=h_prev.device).view(1, 1, 1, W_hidden)

        # # Apply shifts with wrap-around (mod H / mod W)
        # source_y = (grid_y - dy) % H_hidden              # [B,1,H,1]
        # source_x = (grid_x - dx) % W_hidden              # [B,1,1,W]

        # h_shifted = h_prev.gather(2, source_y.expand(-1, C, -1, W_hidden)) \
        #                 .gather(3, source_x.expand(-1, C, H_hidden, -1))
...
class FlowEquivariantRNN(nn.Module):
    def __init__(self, input_channels, hidden_channels, world_size, window_size,
        ...
        if cell_type == 'v_channels':
            ...
            raise ValueError(f"Unknown cell_type: {cell_type}. Must be 'roll' or 'action_concat'")
    ...
    def forward(self, input_seq, input_actions, target_actions, target_seq=None, teacher_forcing_ratio=0.0, return_hidden=False):
        batch, T_in, C, H, W = input_seq.size()
        assert H == self.window_size, "Input sequence height must match window size"
        assert W == self.window_size, "Input sequence width must match window size"
...
class LinearRNNBaseline(nn.Module):
    ...
    def forward(self, input_seq, input_actions=None, target_actions=None, target_seq=None, teacher_forcing_ratio=0.0, return_hidden=False):
